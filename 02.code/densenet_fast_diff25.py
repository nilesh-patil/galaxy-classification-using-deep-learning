...
run = 'densenet_fast_02_64x64_diff25'
device_use = '/gpu:0'
differ = 25
...
print('train_y_expanded shape:', train_y_expanded.shape)
...
batch_size = 50
train_steps = 10*np.int(train_x.shape[0]/batch_size)
...
with tf.device(device_use):  
    for file_path in tqdm(test_files):
        ...
        input_img = np.expand_dims(cv2.resize(cv2.imread(file_path).astype(np.int16),
                                              input_size),
                                   axis=0)
        galaxy_img = (input_img-diff).clip(0)
